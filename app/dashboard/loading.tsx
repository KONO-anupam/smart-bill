      ...
      <main className="max-w-7xl mx-auto px-6 pt-24 pb-16">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            ...
            <div className="h-9 w-56 bg-slate-100 rounded-lg animate-pulse" />
          </div>

          {/* Column headers */}
          <div className="grid grid-cols-6 gap-4 px-6 py-3 bg-slate-50 border-b border-slate-100">
            {[40, 56, 32, 28, 20, 24].map((w, i) => (
              <div
                key={i}
                className={`h-3 w-${w} bg-slate-200 rounded animate-pulse`}
              />
            ))}
          </div>
